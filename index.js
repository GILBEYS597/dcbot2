...
for (const folder of commandFolders) {
   ...
   for (const file of commandFiles) {
    ...
    console.log(`[WARN]: ${filePath} bir valid komut dosyası değildir!`)
}
}
...
client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (!command) {
        ...
        if (interaction.replied || interaction.deferred) {
            await interaction.followUp({ content: 'bu komutu kullanamazsın!', ephemeral: true });

        } else {
            await interaction.reply({ content: 'bu komutu kullanamazsın!', ephemeral: true });
        }
        return;
    }
